...
rclcpp::Subscription<robots_localization::msg::CustomMsg>::SharedPtr sub_pcl_livox;
...
// livox激光雷达回调函数
void livox_pcl_cbk(const robots_localization::msg::CustomMsg::ConstPtr& msg) {
   ...
   public:
    RobotsLocalizationNode() : Node("robots_localization_node") {
        ...
        if (p_lidar->lidar_type == AVIA) {
            sub_pcl_livox =
                this->create_subscription<robots_localization::msg::CustomMsg>(lid_topic, 1, livox_pcl_cbk);
        } else {
            sub_pcl =
                this->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, 1, standard_pcl_cbk);
        }
        ...
        if (log_mocap_traj)
            sub_mocap =
                this->create_subscription<geometry_msgs::msg::PoseStamped>(mocap_topic, 20000, mocap_cbk);
    ...
    void loadConfig() {
        ...
        ransac_average_error_threshold =
            this->get_parameter("uwb.ransac_average_error_threshold").as_double();
    ...
    void initializeUWB() {
        ...
        for (int i = 1; i <= UWB_ANCHOR_NUM; ++i) {
            ...
            anchor_map[i] = anchor;
            anchor_id_state_index[i] =
                26 + 5 * (i - 1);  // 每个uwb anchor状态变量在系统状态向量中的下标起始值
        }
        ...
        if (use_calibrated_anchor) {
            ...
            calibrated_anchor << calibrated_anchor1[0], calibrated_anchor1[1], calibrated_anchor1[2], 1.0,
                0.0;
            uwb_state.anchor1 = vect5(calibrated_anchor);
            anchor_map[1].position = calibrated_anchor.head<3>();
            calibrated_anchor << calibrated_anchor2[0], calibrated_anchor2[1], calibrated_anchor2[2], 1.0,
                0.0;
            uwb_state.anchor2 = vect5(calibrated_anchor);
            anchor_map[2].position = calibrated_anchor.head<3>();
            calibrated_anchor << calibrated_anchor3[0], calibrated_anchor3[1], calibrated_anchor3[2], 1.0,
                0.0;
            uwb_state.anchor3 = vect5(calibrated_anchor);
            anchor_map[3].position = calibrated_anchor.head<3>();
            calibrated_anchor << calibrated_anchor4[0], calibrated_anchor4[1], calibrated_anchor4[2], 1.0,
                0.0;
