...
class RobotsLocalizationNode : public rclcpp::Node {
   public:
    RobotsLocalizationNode() : Node("robots_localization_node") {
        ...
        kf.init_dyn_share(get_f, df_dx, df_dw, RobotsLocalizationNode::h_share_model, NUM_MAX_ITERATIONS, epsi);
        ...
        if (p_lidar->lidar_type == AVIA) {
            sub_pcl_livox =
                this->create_subscription<robots_localization::msg::CustomMsg>(lid_topic, 1, std::bind(&RobotsLocalizationNode::livox_pcl_cbk, this, std::placeholders::_1));
        } else {
            sub_pcl =
