...
#pragma omp parallel for default(none) private(point_num, scan, trans_cloud, kdtree, knn_num, rotation_matrix, H, b)
#endif
        for (size_t i = 0; i < point_num; i++) {
            ...
            // Eigen::aligned_allocator<PointType> 是一个内存分配器，它用于为存储在 std::vector 中的 PointType 对象分配内存。
