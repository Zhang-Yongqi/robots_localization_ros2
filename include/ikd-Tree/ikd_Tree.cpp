...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE **root, PointType point,
  ...
  add_log.point = point;
  Push_Down(*root);
  // 首先确定当前节点的划分轴，然后判断该点应该分到左子树还是右子树
  // 如果在左边则进行左边的比较然后添加
  if (((*root)->division_axis == 0 && point.x < (*root)->point.x) ||
      ((*root)->division_axis == 1 && point.y < (*root)->point.y) ||
      ((*root)->division_axis == 2 && point.z < (*root)->point.z)) {
...
// 当属性Pushdown 为true时，Pushdown 函数将把deleted, treedeleted,
// 和 pushdown的标签复制到它的子节点（但没有复制到它的后代）。
template <typename PointType>
...
// 所有节点数，invalidnum存储子树上标记为“删除”的节点数，range（见数据结构1，第7行）汇总子
// 树上所有点坐标轴的范围，其中k为点维度。
template <typename PointType>
void KD_TREE<PointType>::Update(KD_TREE_NODE *root) {
  ...
  if (left_son_ptr != nullptr && right_son_ptr != nullptr) {
    // 当前树的节点数=左子树节点数+右子树节点数+1(当前节点本身)
  ...
  // 将树的区间拷贝给节点属性 node_range_x = [node_range_x_min,node_range_x_max]
