...
class esekf
{
  ...
  public:
    // typename关键字用于告诉编译器state::scalar是一个类型，而不是一个静态成员变量或函数。这是因为在C++模板中，通常需要使用typename来指明一个嵌套类型。
    typedef typename state::scalar scalar_type;
    typedef Matrix<scalar_type, n, n> cov; // 23X23的协方差矩阵  最终进行正向传播的矩阵Fx_final(因为状态量x是n维的)
                                           // 和下面的cov相比主要是在predict函数中减少了一行
    ...
    typedef Matrix<scalar_type, m, 1> flatted_state;    // 24X1的向量
    ...
    {
        // f函数对应use-ikfom.h中的 get_f 函数，对应fast_lio2论文公式(2)
        flatted_state f_ = f(x_, i_in); // m*1
        // f_x函数对应use-ikfom.h中的 df_dx 函数，对应fast_lio2论文公式(7)
        cov_ f_x_ = f_x(x_, i_in); // 24*23
        cov f_x_final;             // 23*23
...
// Matrix<scalar_type, Eigen::Dynamic, 1> h = h_dyn_share(x_, dyn_share);
            ...
            // 预测得到的误差状态协方差矩阵
            // 协方差矩阵在迭代过程中不会代入下一次迭代，直到最后一次退出时更新，
            // 在迭代过程中更新的只是先验
            P_ = P_propagated;

            // 求协方差的先验更新，大致上是P=(J^-1)*P*(J^-T)如论文式16~18
            Matrix<scalar_type, 3, 3> res_temp_SO3;
            MTK::vect<3, scalar_type> seg_SO3;
            for (std::vector<std::pair<int, int>>::iterator it = x_.SO3_state.begin(); it != x_.SO3_state.end(); it++)
            {
